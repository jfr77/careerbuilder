...
def _job_context(db: Session, pipeline_id=None, job_id=None, pasted=None) -> str:
    """Assemble the job description block from a pipeline entry, a scraped job,
    or pasted text — whichever the request supplies."""
    parts = []
    if pipeline_id:
        entry = get_entry_or_404(db, pipeline_id)
...
def cover_letter(profile_id: int, body: CoverLetterRequest, db: Session = Depends(get_db)):
    ...
    job_ctx = _job_context(db, body.pipeline_id, None, body.pasted_posting)
    prompt = (
...
def cv_tailor(profile_id: int, body: CVTailorRequest, db: Session = Depends(get_db)):
    ...
    job_ctx = _job_context(db, body.pipeline_id, body.job_id, body.pasted_posting)
    prompt = (
...
def interview_trainer(profile_id: int, body: TrainerRequest, db: Session = Depends(get_db)):
    ...
    target = (
        _job_context(db, body.pipeline_id) if body.pipeline_id
